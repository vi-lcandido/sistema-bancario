...
public class NuBank extends Banco {
    ...
    public void abrirConta(Pessoa pessoa) {
        if (pessoa != null) {
            ...
            if (pessoa.getTipoPessoa() == 1) {
                ...
                System.out.println("Apenas tipo 1 ou 2 são permitidos para abrir uma conta no NuBank");
                return;
            }
            ...
            if (novaConta != null) {
                ...
                String numeroConta = "NuBank-" + criarNumeroAleatorio();
    ...
    public boolean encerrarConta(Pessoa pessoa) {
        ...
        if (pessoa != null && conta != null && tipoConta[0].equals("NuBank")) {
            // remove a pessoa da lista de clientes
            clientes.remove(pessoa);

            // define como nula a conta da pessoa
            pessoa.setConta(null);
            System.out.println("Conta encerrada com sucesso no NuBank");
            return true;
        } else {
            System.out.println("Não existe essa conta no NuBank para encerrar");
            return false;
        }
    }
}
